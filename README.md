# Snowflake + AWS Data Pipeline

A batch data pipeline using Python, Apache Airflow, AWS services, Snowflake, and PostgreSQL.

## Documentation

### 1. Project
- [Requirements](docs/01_project/requirements.md)
- [Project Analysis](docs/01_project/analyze.md)

### 2. Specifications
- [Function Specifications - English](docs/02_specs/functions_specs_en.md)
- [Function Specifications - Vietnamese](docs/02_specs/functions_specs_vn.md)

### 3. Architecture
- [System Architecture](docs/03_architecture/architecture.md)
- [Data Flow](docs/03_architecture/dataflow.md)
- [Table Configuration](docs/03_architecture/table_config.md)

> Architecture:
![Architecture](./docs/assets/architecture_snowflake.png)

> Data Flow:
![Data Flow](./docs/assets/dataflow_snowflake.png)

### 4. Code Documentation
- [Helper Functions](docs/04_code/helper_functions.md)
- [Snowflake Functions - English](docs/04_code/snowflake_functions_en.md)
- [Snowflake Functions - Vietnamese](docs/04_code/snowflake_functions_vn.md)